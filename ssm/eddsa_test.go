...
func TestCreateEdDSAKeyPair(t *testing.T) {
	var singer Signer = NewEdDSASigner()
	privKey, pubKey, _, _ := singer.CreateKeyPair()
...
func TestSignEdDSAMessage(t *testing.T) {
	...
	txMsg, _ := singer.SignMessage(privKey, common.Hash{}.String())
...
func TestSignEdDSAMessageV2(t *testing.T) {
	...
	signature, err := singer.SignMessage(fullPrivateKey, message)
...
func TestVerifyEdDSASign(t *testing.T) {
	...
	ok, _ := singer.VerifyMessage(pubKey, common.Hash{}.String(), signature)
	fmt.Println("ok=", ok)
}
